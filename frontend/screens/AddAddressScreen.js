...
const AddAddressScreen = () => {
  ...
  // Define a state for accessToken
  ...
  const fetchAddresses = async () => {
    try {
      const response = await axios.get(`http://192.168.0.101:3001/addresses`, {
        headers: {
  ...
  // Refresh the addresses when the component comes to focus, i.e., when navigating back
  useFocusEffect(
    useCallback(() => {
      if (accessToken !== "") {
        fetchAddresses();
